"""
Automated Daily Update Script for S&P 500 Prediction System

This script runs daily to:
1. Fetch latest S&P 500 prices
2. Generate new prediction
3. Update accuracy for previous predictions
4. Save everything to history

Schedule this to run daily at market close (e.g., 5 PM EST)
"""
...
print("\n" + "="*80)
print("S&P 500 AUTOMATED DAILY UPDATE")
print("="*80)
...
try:
    ...
    latest_data = sp500.history(period='5d')  # Get last 5 days
...
try:
    ...
    if os.path.exists(predictions_file):
        ...
        # Load existing accuracy file or create new
        ...
        if new_records:
            ...
            correct_count = sum([r['is_correct'] for r in new_records])
            accuracy_pct = (correct_count / len(new_records)) * 100

            print(f"[OK] Updated {len(new_records)} predictions with actual results")
            print(f"  Correct: {correct_count}/{len(new_records)} ({accuracy_pct:.1f}%)")
        else:
    ...
    if not os.path.exists(model_path):
        ...
        if result:
            ...
            if os.path.exists(predictions_file):
                ...
                # Check if we already have a prediction for this data_date today
                df['data_date_only'] = pd.to_datetime(df['data_date']).dt.date
                df['prediction_date_only'] = pd.to_datetime(df['prediction_date']).dt.date

                today = datetime.now().date()
                latest_date_only = pd.to_datetime(latest_date).date()

                already_exists = ((df['data_date_only'] == latest_date_only) &
                                 (df['prediction_date_only'] == today)).any()

                if already_exists:
                    print("[OK] Prediction already exists for today - skipping")
                else:
                    df = df.drop(columns=['data_date_only', 'prediction_date_only'])
                ...
                df.to_csv(predictions_file, index=False)
                print(f"[OK] First prediction generated and saved")
        else:
...
try:
    ...
    if os.path.exists('predictions_with_accuracy.csv'):
        acc_df = pd.read_csv('predictions_with_accuracy.csv')
        total = len(acc_df)
        correct = acc_df['is_correct'].sum()
        ...
        acc_df['prediction_date'] = pd.to_datetime(acc_df['prediction_date'], format='mixed')
        recent = acc_df[acc_df['prediction_date'] >= datetime.now() - timedelta(days=30)]
    ...
    if os.path.exists('predictions_history.csv'):
        pred_df = pd.read_csv('predictions_history.csv')
